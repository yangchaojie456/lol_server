module.exports = function () {
    ...
    // 读取召唤师峡谷的列表
    fs.readFile('./format_utils/zhsxg.txt', function (err, data) {
        if (err) {
    ...
    // 读取极地大乱斗的列表
    fs.readFile('./format_utils/jddld.txt', function (err, data) {
        if (err) {
            return console.error(err);
        }
        // console.log("异步读取: " + data.toString().split('\r\n'))
    ...
    // 读取扭曲丛林的列表
    fs.readFile('./format_utils/nqcl.txt', function (err, data) {
        if (err) {
            return console.error(err);
        }
        // console.log("异步读取: " + data.toString().split('\r\n'))
    ...
    let lolItemData = ''
    // 异步读取
    fs.readFile('./temp_json/equipTencent.js', function (err, data) {
        if (err) {
            return console.error(err);
        }
        ...
        fs.writeFile('./temp_json/equip_output.js', decodeUnicode(data.toString()), function (err) {
            if (err) {
                return console.error(err);
            }
            // if (map == 'zhsxg') {
            ...
            //     cMap = 10
            //   }
            var equipAll = require('../temp_json/equip_output.js')
            ...
            fs.writeFile('./data_json/equipList.json', JSON.stringify(equipAll), function (err) {
                if (err) {
                    return console.error(err);
                }
            })
